      ...
      await openTuan();
       ...
       for (let i = 0; i < cookiesArr.length; i++) {
        ...
        for (let code of inviteCodes) {
            ...
            console.log(`\n【${$.UserName}】去助力【${code['user']}】邀请码：${code['assistStartRecordId']}`);
            $.log(code['assistStartRecordId'],code['assistedPinEncrypted'])
            let res = await helpFriendTuan(code['activityIdEncrypted'],code['assistStartRecordId'],code['assistedPinEncrypted'])}
}
...
function helpFriendTuan(activityIdEncrypted='',assistStartRecordId='',assistedPinEncrypted='') {
  return new Promise(async resolve => {
    ...
    let body1 = {"paramData": {"channel": "FISSION_BEAN"}}
    let h5st = h5stSign(body1) || 'undefined'

    //$.log(body)
    
    $.post(taskTuanUrl("vvipclub_distributeBean_assist", body,h5st), async (err, resp, data) => {
     // console.log(`${JSON.stringify(taskTuanUrl("vvipclub_distributeBean_assist", body,h5st))}`)  
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
